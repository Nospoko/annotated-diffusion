...
class Unet(nn.Module):
    def __init__(self, dim: int, dim_mults: tuple[int] = (1, 2, 4, 8), in_channels: int = 3, resnet_block_groups: int = 4):
        super().__init__()
        ...
        # initial conv
        self.init_conv = nn.Conv2d(in_channels, dim, kernel_size=1)
        ...
        # final blocks
        self.final_resnet_block = ResnetBlock(dim * 2, dim, time_emb_dim=self.time_dim, groups=self.resnet_block_groups)
        self.final_conv = nn.Conv2d(dim, in_channels, kernel_size=1)
