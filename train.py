...
class UnetTrainingWrapper(pl.LightningModule):
    def __init__(
            self, model: Unet, forward_diffusion: ForwardDiffusion, lr: float = 1e-3, l2: float = 0.01, 
            loss_type: str = "mse", ckpt_dir: str = "checkpoints", save_every_x_steps: int = 100
        ):
        ...
        torch.save(
            {
                ...
                "forward_diffusion": self.forward_diffusion.state_dict()
            },
            save_path
        )
...
def train(dataloader: DataLoader, args):
    ...
    model = Unet(
        ...
        in_channels=args.in_channels, resnet_block_groups=args.num_resnet_groups
    )

    forward_diffusion = ForwardDiffusion(
        beta_start=args.beta_start, beta_end=args.beta_end, 
        timesteps=args.timesteps, schedule_type=args.schedule_type
    )
    ...
    model_training_wrapper = UnetTrainingWrapper(
        model, forward_diffusion, lr=args.lr, l2=args.l2, 
        loss_type=args.loss_type, ckpt_dir=args.save_ckpt_dir, save_every_x_steps=args.save_every_x_steps
    )
    ...
    trainer = pl.Trainer(
        logger=logger, callbacks=callbacks, max_epochs=args.num_epochs, 
        accelerator=args.accelerator, precision=args.precision, 
        log_every_n_steps=args.log_every_n_steps, overfit_batches=args.overfit_batches
    )
...
def parse_args():
    ...
    # unet parameters
    parser.add_argument("--dim", type=int, default=32, help="Initial number of Unet channels (This number will be multiplied by dim_mults)")
    parser.add_argument("--dim_mults", type=tuple[int], default=(1, 2, 4), help="Multipliers of dim for each layer. It also specifies network depth.")
    ...
    parser.add_argument("--schedule_type", type=str, choices=["cosine", "linear", "quadratic", "sigmoid"], default="cosine", help="Schedule type.")
